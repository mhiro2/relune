...
`--db-url` introspection runs catalog queries through a small connection pool. Set `RELUNE_DB_POOL_MAX_CONNECTIONS` to a positive integer to override the default cap (PostgreSQL/MySQL fan out to 7 connections, SQLite uses 1). Non-positive or non-numeric values are ignored and the default applies.
