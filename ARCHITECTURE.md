...
Phases: build layout graph → grouping/focus → layout algorithm → coordinates → **auto-tune spacing** → **global port assignment** → **obstacle-aware channel selection** → **self-loop/debug detour fallback** → **label collision avoidance** → bounds. Handles cycles, join tables, views, enum references, and multi-schema namespacing.

**Routing model** — Layout returns one canonical orthogonal backbone per edge. Hierarchical routing uses `port -> stub -> channel -> stub -> port`, then scores inter-rank, same-rank, and reverse-flow channel candidates in a deterministic greedy edge order. Candidate scoring combines hard obstacle constraints with weighted soft costs for clearance, route length, bend count, center deviation, and channel congestion. Renderers own the final visual style (`straight`, `orthogonal`, `curved`) so route topology does not depend on the selected edge style.

**Quality passes** — After backbone routing, `nudge_label` shifts edge labels away from overlapping nodes. `detour_around_obstacles` is no longer part of the default non-self-loop path; it remains for self-loop handling and an explicit debug fallback gated by `RELUNE_ENABLE_DETOUR_FALLBACK`, while routing keeps an internal activation count for edges that would still require detour. Additionally, `auto_tuned` adjusts horizontal/vertical spacing based on node count and edge density before coordinate assignment, and port slot offsets keep parallel edges stable on each node side.
