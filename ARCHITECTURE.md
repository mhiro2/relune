...
**Placement model** — Ranks come from longest-path layering after breaking foreign-key cycles with a greedy (Eades–Lin–Smyth) feedback arc set, so a dense cycle spans only the layers its acyclic skeleton needs. Nodes are then split into clusters: weakly connected components, merged with their group so a group container never spans two clusters. Inside a cluster, groups sit side by side as lanes that share the cluster's rank rows, so edges between groups still flow along the primary axis, and tables without any edge are packed into a grid below their lane. Clusters are wrapped into shelves along the secondary axis, sized toward a screen-friendly aspect ratio but never narrower than the widest cluster, so a single connected schema keeps its original placement. Routing uses the packed row index as the rank, and rows are disjoint bands along the primary axis. In force-directed mode, the packing pass only separates items that share a rank row, are directly linked, or sit next to a self-loop, so unrelated tables on different ranks can share a column; routing rows are then recomputed as disjoint bands of the final primary-axis extents.
